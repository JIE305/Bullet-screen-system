...
数据库由 SQLAlchemy 2.0 管理，并通过 Alembic `0001_initial` 至 `0005_remove_legacy_victory_rule` 迁移。生产文件位于 Electron 用户数据目录的 `runtime/damusystem.sqlite3`。
...
| `danmaku_rules` | id、可空 profile_id、match_type、pattern、template、confidence、cooldown_ms、priority | 全局规则仅作历史兼容且不再读取；非空配置规则供内置测试使用 | 永久 |
