...
const emit = defineEmits<{
  ...
  tested: [result: CloudApiTestResult]
}>()
...
const error = ref('')
const result = ref<CloudApiTestResult>()
...
async function saveAndTest(): Promise<void> {
  ...
  try {
    await save()
    const tested = await damuApi.testCloudApi()
    result.value = tested
    emit('tested', tested)
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : String(caught)
    ...
    <Transition name="cloud-drawer-fade">
      <div v-if="open" class="cloud-drawer-layer">
        ...
        >
          ...
          <div v-if="result" class="cloud-result" aria-live="polite">
            <span>TEST RESULT · {{ Math.round(result.elapsedMs) }} MS</span>
            <strong>{{ result.text }}</strong>
          </div>

          <form class="cloud-form" @submit.prevent="saveOnly">
            <label class="enable-row">
              <span><b>启用云端生成</b><small>关键词命中后才会调用</small></span>
            ...
            <div class="cloud-number-grid">
              ...
              <label><span>每分钟上限</span><input v-model.number="draft.maxCallsPerMinute" type="number" min="1" max="60" step="1" :disabled="sessionActive" /><small>{{ draft.maxCallsPerMinute }} 次</small></label>
            </div>

            <section class="privacy-note">
              <strong>PRIVACY GATE</strong>
              <p>只发送命中关键词的 OCR 文本和本地模板结果；不发送截图、窗口标题或完整事件历史。调用失败会立即使用本地模板。</p>
            </section>

            <footer>
              <button class="cloud-test" type="button" :disabled="busy || sessionActive" @click="saveAndTest">{{ busy ? '处理中' : '保存并测试“胜利”' }}</button>
              <button class="cloud-save" type="submit" :disabled="busy || sessionActive">保存配置</button>
            </footer>
          </form>
        </aside>
      </div>
...
.cloud-lock, .cloud-warning, .cloud-error, .cloud-result { margin: 16px 24px 0; padding: 11px 12px; border: 1px solid #526058; border-radius: 6px; font-size: 10px; line-height: 1.55; }
...
.cloud-result { display: grid; gap: 7px; border-color: color-mix(in srgb, #a7e46b 55%, #354139); }.cloud-result span { color: #a7e46b; font: 500 9px 'JetBrains Mono', monospace; }.cloud-result strong { font-size: 13px; font-weight: 500; }
...
.cloud-number-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }.cloud-number-grid label { position: relative; }.cloud-number-grid small { position: absolute; right: 9px; bottom: 12px; pointer-events: none; }.cloud-number-grid input { padding-right: 48px !important; }
...
.cloud-form footer { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding-top: 4px; }.cloud-test { color: #a7e46b; }.cloud-save { color: #101512; border-color: #a7e46b; background: #a7e46b; font-weight: 600; }
